...
void readAlgorithmResults()
{
   ...
   uint8_t data_rq[4];
   uint8_t status = HAL_I2C_Mem_Read(&hi2c, CCS811_ADDRD, (uint8_t)CSS811_ALG_RESULT_DATA, I2C_MEMADD_SIZE_8BIT, data_rq, 4,
       100);
...
void configureCCS811()
{
   ...
   uint8_t hwID;// = readRegister(0x20); //Hardware ID should be 0x81
	 HAL_I2C_Mem_Read_IT(&hi2c, CCS811_ADDRD, (uint8_t)addr, 1, &hwID, 1);
   ...
   HAL_I2C_Master_Transmit(&hi2c, 0xB6, lodata, 1, 100);
...
void setEnvironmentalData(float relativeHumidity, float temperature)
{
   ...
   env[3] = envData[2];
   env[5] = envData[3];
   HAL_I2C_Mem_Write(&hi2c, CCS811_ADDWR, (uint8_t)CSS811_ENV_DATA, I2C_MEMADD_SIZE_8BIT, envData, 4,
       100);
   while (HAL_I2C_GetState(&hi2c) != HAL_I2C_STATE_READY)
   {
   }
   while (HAL_I2C_IsDeviceReady(&hi2c, CCS811_ADDRD, 10, 300) == HAL_TIMEOUT)
      ;
   while (HAL_I2C_GetState(&hi2c) != HAL_I2C_STATE_READY)
   {
   }
...
{
   HAL_I2C_Mem_Read(&hi2c, CCS811_ADDRD, (uint8_t)addr, I2C_MEMADD_SIZE_8BIT, val, size, 100);
}
...
void softRest()
{
   ...
   HAL_I2C_Mem_Write(&hi2c, CCS811_ADDWR, CSS811_SW_RESET, I2C_MEMADD_SIZE_8BIT, rstCMD, 5, 300);
   while (HAL_I2C_GetState(&hi2c) != HAL_I2C_STATE_READY)
      ;
}
...
uint32_t get_Sensor_Resistance()
{
   ...
   HAL_I2C_Mem_Read(&hi2c, CCS811_ADDRD, CSS811_RAW_DATA, I2C_MEMADD_SIZE_8BIT, sensor_Resistance_raw, 2, 100);
...
uint8_t readRegister(uint8_t addr)
{
   ...
   HAL_I2C_Mem_Read_IT(&hi2c, CCS811_ADDRD, (uint8_t)addr, 1, &dt, 1);
   while (hi2c.State != HAL_I2C_STATE_READY)
      ;
...
void restore_Baseline()
{
   ...
   HAL_I2C_Mem_Write(&hi2c, CCS811_ADDWR, CSS811_BASELINE, I2C_MEMADD_SIZE_8BIT, res_bs, 2, 300);
   while (HAL_I2C_GetState(&hi2c) != HAL_I2C_STATE_READY)
   {
   }
}
...
unsigned int getBaseline()
{
   ...
   status = HAL_I2C_Mem_Read(&hi2c, CCS811_ADDRD, (uint8_t)CSS811_BASELINE, 1, ada, 2, 100);
   while (HAL_I2C_GetState(&hi2c) != HAL_I2C_STATE_READY)
   {
   }
   while (HAL_I2C_IsDeviceReady(&hi2c, CCS811_ADDRD, 10, 300) == HAL_TIMEOUT)
...
void writeRegister(uint8_t addr, uint8_t val)
{
   ...
   HAL_I2C_Mem_Write(&hi2c, CCS811_ADDWR, (uint8_t)addr, I2C_MEMADD_SIZE_8BIT, &val, 1, 300);
   while (HAL_I2C_GetState(&hi2c) != HAL_I2C_STATE_READY)
   {
   }
   while (HAL_I2C_IsDeviceReady(&hi2c, CCS811_ADDRD, 10, 300) == HAL_TIMEOUT)
